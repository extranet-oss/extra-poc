...
def update():
    ...
    try:
        ...
        log.fail(f'Failed to load ressource:\n{traceback.format_exc()}')
        print(log.msg)
        return
    ...
    try:
        ...
        for country, cities in tree.items():
            ...
            for city, buildings in cities.items():
                ...
                for building, rooms in buildings.items():
                    ...
                    for room in rooms:
                        key = f'{country}/{city}/{building}/{room}'
                        if 'disabled' in locations[key] and locations[key]['disabled']:
                            continue
        ...
        db.session.rollback()
        log.fail(f'Failed to process data:\n{traceback.format_exc()}')
        print(log.msg)
        return
    ...
    try:
        ...
        db.session.rollback()
        log.fail(f'Failed to delete outdated data:\n{traceback.format_exc()}')
        print(log.msg)
        return
