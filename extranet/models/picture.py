...
PICTURE_DIR = f'{app.static_folder}/{PICTURE_STATIC_DIR}'
FORMAT = 'jpg'
FORMAT_SETTINGS = {
    'progressive': True,
    'quality': 95
...
def save_picture(uuid, image):
    os.makedirs(f'{PICTURE_DIR}/', exist_ok=True)

    # save original image
    image = image.convert('RGB')
    image.save(f'{PICTURE_DIR}/{uuid}.{FORMAT}',
               **FORMAT_SETTINGS)

    # save thumbnails
    for size in SIZES:
        os.makedirs(f'{PICTURE_DIR}/{str(size[0])}x{str(size[1])}/', exist_ok=True)
        ...
        resized.save(f'{PICTURE_DIR}/{str(size[0])}x{str(size[1])}/{uuid}.{FORMAT}',
                     **FORMAT_SETTINGS)
...
class Picture(Base):
    ...
    def get_url(self, size=None):
        ...
        if size is not None and [size[0], size[1]] in self.sizes:
            subdir = f'{str(size[0])}x{str(size[1])}/'

        return url_for('static', filename=f'{PICTURE_STATIC_DIR}/{subdir}{self.uuid}.{FORMAT}')
...
def before_delete_event(mapper, connection, target):
    filename = f'{PICTURE_DIR}/{target.uuid}.{FORMAT}'
    ...
    for size in target.sizes:
        filename = f'{PICTURE_DIR}/{str(size[0])}x{str(size[1])}/{target.uuid}.{FORMAT}'
