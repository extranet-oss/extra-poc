...
class Intranet():
    ...
    useragent = f"Mozilla/5.0 (compatible; {APP_NAME}/{version_tostring(APP_VERSION)}; +{external_url('/bot')})"
    ...
    def request(self, url, data=None, format='json', method='GET', token=None, raw=False):
        ...
        target = f'{self.base_url}{token}/{url}'
        ...
        if not r.ok:
            error_message = f'HTTP/{r.status_code} "{r.reason}" recieved'
    ...
    def get_picture(self, login=None, **kwargs):
        ...
        url = f"https://cdn.local.epitech.eu/userprofil/{login.split('@')[0]}.bmp"
        ...
        if not r.ok:
            if r.status_code == 404:
                ...
                raise IntranetError(f'HTTP/{r.status_code} "{r.reason}" recieved')
    ...
    def set_user_profile(self, data, login=None, **kwargs):
        ...
        for name, info in data.items():
            payload[f'{name}[name]'] = name
            payload[f'{name}[value]'] = info['value']

            if 'public' in info and info['public']:
                payload[f'{name}[public]'] = 'on'
            else:
                payload[f'{name}[public]'] = ''

            if 'adm' in info and info['adm']:
                payload[f'{name}[adm]'] = 'on'
            else:
                payload[f'{name}[adm]'] = ''
        kwargs['data'] = payload

        return self.post(f'user/{login}/edit/save', **kwargs)

    def get_group(self, name, **kwargs):
        return self.get(f'group/{name}/', **kwargs)

    def get_group_members(self, name, **kwargs):
        return self.get(f'group/{name}/member/', **kwargs)
